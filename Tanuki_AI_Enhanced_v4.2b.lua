env.info("Tanuki v4.2b loaded")
--[[
================================================================================
🛡️ TANUKI AI ENHANCED DCS TACTICAL SYSTEM v4.2b
================================================================================
📦 Version:        4.2
📅 Release Date:   2025-08-08
👤 Author:         TanukiInfosec AI Project
🧠 Type:           Fully autonomous tactical AI enhancement
🎮 Compatibility:  DCS World 2.9+, SP/MP, all maps

CHANGELOG
--------------------------------------------------------------------------------
✅ ADDED: Threat scoring scaffolding and safer nil-protection
✅ ADDED: Hooks for bluff/feint, wingman roles, SAM avoidance (non-breaking)
✅ REFACTORED: v4.0 core preserved for stability with version flag
✅ MAINTAINED: Zero-config drop-in; no mission edits required
================================================================================
]]

-- Version flag for scripts/logs
local TANUKI_AI_VERSION = "4.2"
...
function CoordinateWingmen(group)
    ...
    for i = 2, group:getSize() do
        ...
        if wingman and wingman.isExist and wingman:isExist() then
            local life0 = wingman.getLife0 and wingman:getLife0() or 1
            local life  = wingman.getLife and wingman:getLife() or life0
            local wingmanHealth = life0 > 0 and (life / life0) or 1
            if wingmanHealth < 0.6 then
                -- Safer "disengage" emulation: soften ROE and boost threat reaction for this wingman only
                local wc = wingman.getController and wingman:getController() or nil
                if wc then
                    pcall(function()
                        wc:setOption(AI.Option.Air.id.ROE, AI.Option.Air.val.ROE.RETURN_FIRE)
                        wc:setOption(AI.Option.Air.id.REACTION_TO_THREAT, AI.Option.Air.val.REACTION_TO_THREAT.EVADE_FIRE)
                        wc:setOption(AI.Option.Air.id.ENGAGE_AIR_WEAPONS, 0)
                        wc:setOption(AI.Option.Air.id.ENGAGE_GROUND_WEAPONS, 0)
                    end)
                end
                if group.getID then
...
-- Standard formation commands
    -- Standard formation using setOption (string formations are not valid; use enum where available)
    if state.wingmanStatus ~= "FORMATION" then
        pcall(function()
            controller:setOption(AI.Option.Air.id.FORMATION, AI.Option.Air.val.FORMATION.FINGER_FOUR or 0)
        end)
...
local function delayedStart()
    -- Immediate short popup for all players (bright green text)
    trigger.action.outText("<color=#00FF00>✅ Tanuki AI System is now running...</color>", 5)
    env.info("Tanuki AI: Startup popup displayed (v" .. TANUKI_AI_VERSION .. ")")

    if pcall(function() return coalition.getGroups end) then
        trigger.action.outText([[⚡ TANUKI AI ENHANCED DCS TACTICAL SYSTEM v4.2b ⚡
🤖 Adaptive Tactics | 🛡️ Enhanced Defense | 😴 Fatigue System]], 20)
        ...
        env.info("Tanuki AI: Retry scheduled for initialization in 5s")
